...
// Fbn generates fratal brownian motion noise
func Fbm(x, y, frequency, lacurnarity, gain float32, octaves int) float32 {
	var sum float32 = 0
	amplitude := float32(1.0)

	for i := 0; i < octaves; i++ {
		sum = sum + Snoise2(x*frequency, y*frequency)*amplitude
		frequency = frequency * lacurnarity
...
func MakeNoise(noiseType NoiseType, frequency, lacurnarity, gain float32, octaves int, w, h int) (noise []float32, min, max int) {
		...
		go func(i int) {
			...
			for j := start; j < end; j++ {
				...
				if noiseType == TURBULENCE {
					...
					noise[j] = Fbm(float32(x), float32(y), frequency, lacurnarity, gain, octaves)
				}

				// Possible race condition
				if noise[j] < float32(min) || noise[j] > float32(max) {
					mutex.Lock()

					if noise[j] < float32(min) {
						min = int(noise[j])
					} else if noise[j] > float32(max) {
						max = int(noise[j])
					}
					mutex.Unlock()
				}
...
func Turbulence(x, y, frequency, lacurnarity, gain float32, octaves int) float32 {
	var sum float32
	amplitude := float32(1)
